...
export function getNode(subdomain: string, parentNode: `0x${string}`): string {
    return keccak256(encodePacked(['bytes32', 'bytes'], [parentNode, keccak256(toHex(subdomain))]));
}
...
export function getParentNode(domain: string): string {
    ...
    const subnode = keccak256(encodePacked(['bytes32', 'bytes'], [zeroBytes32Hex, keccak256(parentDomainHex)]));

    return keccak256(encodePacked(['bytes32', 'bytes'], [subnode, keccak256(toHex(domain))]));
}
