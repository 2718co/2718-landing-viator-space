...
export default function Home() {
    ...
    useEffect(() => {
        if (!prefersReducedMotion) {
            const interval = setInterval(() => setCurrentIdx((prev) => prev + 1), GALLERY_SLIDESHOW_INTERVAL);
            ...
            <div className="grid grid-flow-row gap-x-10 gap-y-2 lg:grid-cols-2 lg:gap-y-8 ">
                ...
                <div className="col-span-1 flex flex-col overflow-hidden rounded-3xl bg-light-background lg:col-span-2">
                    <AnimatePresence initial={false} mode="wait">
                        <div id="gallery" className="relative flex h-[544px] cursor-pointer flex-row">
                            {/* Main Image */}
                                ...
                                {typeof window !== 'undefined' &&
                                    createPortal(
                                        <Lightbox
                                            ...
                                            imgUrls={[bigImage, tallImage, hangingImage, squareLeft, squareRight]}
                                        />,
                                        document.body
                                    )}
                    ...
                    <div className="flex flex-col bg-none pb-2 pt-9 lg:flex-row lg:px-5 lg:py-9">
                        {/* Description */}
                        <div className="flex flex-1 flex-col space-y-1 px-2 pb-2 lg:px-0 lg:pb-0">
                            <span className="font-mono text-pretitle-size uppercase">Genesis collection</span>
                            <h1 className="font-mono text-title-1-size font-medium">Viator TRAPPIST-1 Mission</h1>
                            <span>
                                <p className="max-w-[58ch] font-mono text-xs leading-[15px] text-light-text">
                                    Viator is the autonomous space probe exploring the TRAPPIST-1 solar system in search
                                    for distant relics. This shirts represents one from 27 limited genesis NFT&apos;s
                                    from the 2718 collective.
                                </p>
                ...
                <div className="col-span-1 overflow-hidden rounded-3xl bg-black">
                    <video src="/ai-art.mp4" autoPlay muted className="h-full w-full object-cover" />
                </div>
